...
export async function GET(_request: NextRequest) {
  try {
    ...
    const training = (await query(
      "SELECT * FROM training WHERE characterId = ?",
      [user.characterId]
    )) as any[];
    ...
    const elapsedMinutes = Math.floor(
      (now.getTime() - lastClaim.getTime()) / (1000 * 60)
    );
...
export async function PUT(_request: NextRequest) {
  try {
    ...
    const characters = (await query(
      "SELECT level, dexterity, intelligence, strength FROM characters WHERE id = ?",
      [user.characterId]
    )) as any[];
    if (characters.length === 0) {
      return NextResponse.json(
    ...
    await query("DELETE FROM training WHERE characterId = ?", [
      user.characterId,
    ]);
    ...
    return NextResponse.json(
      { error: "Failed to claim training" },
      { status: 500 }
    );
  }
}
